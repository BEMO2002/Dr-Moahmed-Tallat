...
export async function generateMetadata({ params }) {
  ...
  const currentCategory = categories.find(
    (c) => c.slug[locale] === decodeURIComponent(category) || c.slug["en"] === decodeURIComponent(category)
  );

  return {
    title: currentCategory ? currentCategory.name[locale] || currentCategory.name["en"] : "Category",
  };
}
...
export default async function CategoryPage({ params }) {
  ...
  const currentCategory = categories.find(
    (c) => c.slug[locale] === decodedCategory || c.slug["en"] === decodedCategory
  );
  ...
  return (
    <div className="bg-white min-h-screen">
      <ArticlesandColumnsHeader
        ...
        breadcrumbCurrent={currentCategory.name[locale] || currentCategory.name["en"]}
        locale={locale}
        isRTL={isRTL}
      />
